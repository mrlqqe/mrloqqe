...
class atlas_i2c:
    long_timeout = 20  # the timeout needed to query readings & calibrations
    short_timeout = .5  # timeout for regular commands
    default_bus = 1  # the default bus for I2C on the newer Raspberry Pis,
                     # certain older boards use bus 0
    default_address = 100  # the default address for the Temperature sensor
...
def main():
    device = atlas_i2c(100)  # creates the I2C port object, specify the address
                          # or bus if necessary
    i=0
    timestamp = 0
...
while(1):
    ...
    # get device reading
    EC = device.query("R")
    Temp = device1.query("R")
    pH = device2.query("R")
    # build the payload string.
    payload = "field1=" + str(EC) + "field2=" + str(Temp)
    ...
    try:
        ...
        print (" Published EC = ", EC, " to host: " , mqttHost , " clientID= " , clientID)
